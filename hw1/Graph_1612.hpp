...
class Graph {
 ...
 public:
  ...
  Edge add_edge(const Node& a, const Node& b) {
	...
	std::vector<size_type> adj_list_a = internal_node_a.adj_list;
	for (size_type i = 0; i < adj_list_a.size(); i++) {
	  if (edges_[adj_list_a[i]].node1_uid == b.index()) {return Edge(this, adj_list_a[i]);}
	  if (edges_[adj_list_a[i]].node2_uid == b.index()) {return Edge(this, adj_list_a[i]);}
	}	
  ...
  class NodeIterator : private equality_comparable<NodeIterator> {
   ...
   private:
    friend class Graph;
    // HW1 #2: YOUR CODE HERE
	NodeIterator (const Graph* graph, size_type uid) : graph_(const_cast<Graph*>(graph)), pos_(uid) {}
	Graph* graph_;
	size_type pos_;
  };
  ...
  class IncidentIterator : private equality_comparable<IncidentIterator> {
   public:
	...
	// Operator == 
	bool operator==(const IncidentIterator& it) const {return (pos_ == it.pos_ and node_uid_ == it.node_uid_);}

   private:
    friend class Graph;
    // HW1 #3: YOUR CODE HERE
	IncidentIterator (const Graph* graph, size_type uid, size_type node_uid) : graph_(const_cast<Graph*>(graph)), pos_(uid), node_uid_(node_uid) {}
	Graph* graph_;
	size_type pos_;
	size_type node_uid_;
  };
  ...
  class EdgeIterator : private equality_comparable<EdgeIterator> {
   ...
   private:
	...
	Graph* graph_;
	size_type pos_;
	EdgeIterator (const Graph* graph, size_type uid) : graph_(const_cast<Graph*>(graph)), pos_(uid) {}
  };
