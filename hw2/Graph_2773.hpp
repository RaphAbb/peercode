...
class Graph {
 ...
 public:
   ...
   * @return 1 if has_node(@a node) else 0
   * @post new num_nodes() == old num_nodes() - 1
   * @post new num_edges() == old num_edges() - @a node.degree()
   *
  ...
  size_type remove_node(const Node& node){
    ...
    size_type deg = node.degree();
    
    auto iter = node.edge_begin();
    ...
    node_values.erase(node_values.begin() + idx);
    return 1;
  }

  /** Remove a node from the graph using its position
   * @pre @ iter must be valid
   * @param[in] iter Iterator that points to the node to be removed
   * @return 1 if has_node(@a node) else 0
   * @post new num_nodes() == old num_nodes() - 1
   * @post new num_edges() == old num_edges() - @a node.degree()
   *
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    edge_value_type& value(){	
      return graph->edge_values[node1().idx*node2().idx];
    }
     ...
     */
    bool operator<(const Edge& e) const { 
      return graph == e.graph && (node_1_idx^node_2_idx) < (e.node_1_idx^e.node_2_idx);
    }
   ...
   * @param[in] a First node in the edge to be removed
   * @param[in] b Second node in the edge to be removed
   * @return 1 if has_edge(@a a, @a b) else 0
   * @post new num_edges() == old num_edges() - 1 if edge exists
   * @post all iterators are invalidated 
   *
  ...
  size_type remove_edge(const Node& a, const Node& b){
    ...
    for(size_type i = 0; i < edges[a.idx].size(); i++){
       if(edges[a.idx][i] == b.idx){
         edges[a.idx].erase(edges[a.idx].begin() + i);
         break;
       }
    }


    for(size_type i = 0; i < edges[b.idx].size(); i++){
       if(edges[b.idx][i] == a.idx){
         edges[b.idx].erase(edges[b.idx].begin() + i);
         break;
       }
    }
   ...
   * @post all iterators are invalidated 
   *
   * Complexity: O(max(@a edge.node1().degree(), @a edge.node2().degree())) 
   */
  ...
  /** Remove a node from the graph using its position
   * @param[in] iter that points to the edge to be removed
   * @return updated iterator
   * @post new num_edges() == old num_edges() - 1
   *
   * Complexity: O(max(@a *iter.node1().degree(), @a *iter.node2().degree())) 
   */
 ...
 private:
  ...
  unsigned long hashing_func(std::pair<size_type, size_type> e){
    return e.first^e.second;
  };

  bool key_equal(std::pair<size_type, size_type> e1, 
                  std::pair<size_type, size_type> e2){
