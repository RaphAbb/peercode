...
class Graph {
 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    /** makes node position modifiable **/
    Point& position() {
  ...
  /**
  * @brief removes a node from the graph along with its incident edges
   *
   * @param n: any Node object
   * @return 1 if node was in the graph and removed, 0 otherwise
   *
   * @pre any node
   * @post returns 1 iff the node was a valid node in the graph
   *        invalidates all node indices and edge indices
   *
   * runtime: O(n * (n + m))
   */
  size_type remove_node(const Node& n) {
    //check that the node is actually in the graph
    if (n.index_ >= this->num_nodes()) {
      return 0;
    }
    //debugging
    /*
    ...
    */
    //remove each incident edge from the graph
    auto deg = n.degree();
    for (size_type i = 0; i < deg; i++) {
      //std::cout << "(" << (*n.edge_begin()).node1().index() << ", " << (*n.edge_begin()).node2().index() << ")\n";
      remove_edge((*n.edge_begin()).node1(), (*n.edge_begin()).node2());
    }
    //std::cout << n.degree() << std::endl;

    //reindex all the edges
    for (size_type i = 0; i < this->num_nodes() - 1; i++) {
      if (i >= n.index_) {
        std::unordered_map<size_type, size_type> correct_map = edges.at(i + 1);
        for (unsigned j = n.index_ + 1; j < this->num_nodes(); j++) {
          if (correct_map.find(j) != correct_map.end()) {
            //reindex each incident edges to i
            auto temp_idx = correct_map.at(j);
            correct_map.erase(j);
            correct_map.insert({j - 1, temp_idx});

            //reindex edge_idxs
            edge_idxs[temp_idx] = std::make_pair(i, j - 1);
          }
        }
        edges.erase(i);
        edges.insert({i, correct_map});
      }
      else {
        for (unsigned j = n.index_ + 1; j < this->num_nodes(); j++) {
          if (edges.at(i).find(j) != edges.at(i).end()) {
            auto temp_idx = edges.at(i).at(j);
            edges.at(i).erase(j);
            edges.at(i).insert({j - 1, temp_idx});
    ...
    */
    ...
    points.erase(points.begin() + n.index_);
    values.erase(values.begin() + n.index_);
    return 1;
  }
  ...
  * @brief removes a node from the graph along with its incident edges
   *
   * @param n_it: any Node iterator
   * @return 1 if node was in the graph and removed, 0 otherwise
   *
   * @pre iterator referring to any node
   * @post returns 1 iff the node was a valid node in the graph
   *        invalidates all node indices and edge indices
   *
   * runtime: O(deg(n) * (m))
  ...
  * @brief removes an edge from the graph
   *
   * @param a, b: any two Node objects
   * @return 1 if edge was in the graph and removed, 0 otherwise
   *
   * @pre any node
   * @post returns 1 iff the node was a valid node in the graph
   *        invalidates all edge indices
   *
   * runtime: O(m)
   */
  size_type remove_edge(const Node& a, const Node& b) {
    if (this->has_edge(a, b)) {
      //find index so we can remove it from our index/value vectors
      size_type index = edges.at(a.index_).at(b.index_);
      //std::cout << index << " " << edges.at(b.index_).at(a.index_) << std::endl;
      //erase edges in our "adjacency list" that's actually a nested map
      edges.at(a.index_).erase(b.index_);
      edges.at(b.index_).erase(a.index_);
      // reindex all the edges
      for (size_type i = index + 1; i < this->num_edges(); i++) {
        size_type n1_idx = edge_idxs[i].first;
        size_type n2_idx = edge_idxs[i].second;
        //std::cout << i << std::endl;
        //std::cout << edges.at(n1_idx).at(n2_idx) << std::endl;
        //std::cout << edges.at(n2_idx).at(n1_idx) << std::endl;
        //std::cout << "on edge: " << n1_idx << ", " << n2_idx << std::endl;
        edges.at(n1_idx).at(n2_idx) = i - 1;
        edges.at(n2_idx).at(n1_idx) = i - 1;
      }
      //erase this edge from our vector info by index
      edge_idxs.erase(edge_idxs.begin() + index);
      edge_values.erase(edge_values.begin() + index);
      /*
      for (unsigned i = 0; i < edge_idxs.size(); i++) {
        std::cout << i << " :: " << edge_idxs[i].first << ", " << edge_idxs[i].second << " " << edges.at(edge_idxs[i].first).at(edge_idxs[i].second) << std::endl;
      }
      */
      return 1;
    }
  ...
  * @brief removes an edge from the graph
   *
   * @param e_it any edge iterator
   * @return 1 if edge was in the graph and removed, 0 otherwise
   *
   * @pre any edge iterator
   * @post returns 1 iff the node was a valid node in the graph
   *        invalidates all edge indices
   *
   * runtime: O(m)
