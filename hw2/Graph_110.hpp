...
class Graph {
 ...
 public:
  ...
  class Node : private totally_ordered<Node> {
    ...
    */
    ...
    bool operator<(const Node& n) const {
      // HW0: YOUR CODE HERE
      return (n.graph==graph)? index() < n.index() : graph<n.graph;
    }
  ...
  class Edge : private totally_ordered<Edge> {
   public:
     ...
     */
    bool operator<(const Edge& e) const {
      return (e.graph==graph) ? idx < e.idx : graph<e.graph;
    }
    ...
    edge_value_type& value(){
        return graph->edge_values[idx];
    }
      
    /** return edge value const ref */
    const edge_value_type& value() const{
        return graph->edge_values.at(idx);
    }
  ...
  */
  ...
  class IncidentIterator : equality_comparable<IncidentIterator>{
   ...
   private:
    ...
    IncidentIterator(graph_type* g, size_type node_idx, SetIter adj_set_iter) 
  ...
  class EdgeIterator  : equality_comparable<EdgeIterator>{
    ...
    */
    bool operator==(const EdgeIterator& other) const{
        return other.idx == idx && other.graph == graph;
    }
  ...
  */
  ...
  size_type remove_node ( const Node & n){
    ...
    // erase node in datastructures
    node_list.erase(node_list.begin()+nidx);
    point_list.erase(point_list.begin()+nidx);
    node_values.erase(node_values.begin()+nidx);
    adjList.erase(adjList.begin()+nidx);
      
    // update reverse index map
    for(size_type i =nidx; i<node_list.size();++i)
        node_list[i].idx -=1;
      
    // clear edge map and adj sets
    for(size_type i =0; i<adjList.size();++i)
        adjList[i].clear();
    edge_map.clear();
      
    // fix auxillary data structures
    for(size_type i =0; i<edge_list.size();++i){
        // fix edge to nodes map
        edge_list[i].n1 -= (edge_list[i].n1>nidx)? 1 : 0;
        edge_list[i].n2 -= (edge_list[i].n2>nidx)? 1 : 0;

        auto aidx = edge_list[i].n1;
        auto bidx = edge_list[i].n2;

        Pair pr1(aidx,bidx);
        Pair pr2(bidx,aidx);
        
        Edge edge(aidx,bidx,i, this);
        
        // fix nodes to edge map
        edge_map[pr1]=edge;
        edge_map[pr2]=edge;
        // fix adj sets
        adjList[aidx].insert(bidx);
  ...
  node_iterator remove_node( node_iterator n_it ){
      return node_iterator(this,remove_node(*n_it));
  }
  ...
  size_type remove_edge(const Node& a, const Node& b){
    ...
    // check if valid call
    if( aidx == INVALID || bidx == INVALID || aidx ==bidx || edge_map[pr1].index() == INVALID){
        std::cout<<"Cant delete edge between node indices : "<<aidx<<", "<<bidx<<std::endl;
        return num_edges();
    }
    
    auto eidx = edge_map[pr1].index();
      
    // erase in primary data structures
    edge_list.erase(edge_list.begin()+eidx);
    edge_map.erase(pr1);
    edge_map.erase(pr2);
      
    // fix auxillary data structures
    for(auto i = edge_list.begin()+eidx; i!=edge_list.end();++i){
        (*i).idx -=1;
        size_type i_aidx = (*i).node1().index();
        size_type i_bidx = (*i).node2().index();
        Pair ipr1(i_aidx,i_bidx);
        Pair ipr2(i_bidx,i_aidx);
        edge_map[ipr1].idx -=1;
        edge_map[ipr2].idx -=1;
    }
    adjList[aidx].erase(bidx);
    adjList[bidx].erase(aidx);
    edge_values.erase(edge_values.begin()+eidx);
  ...
  edge_iterator remove_edge ( edge_iterator e_it ){
      return edge_iterator(this,remove_edge(*e_it));
  }
