 ...
 public:
  ...
  class Edge {
   public:
    ...
    bool operator==(const Edge& e) const {
      ...
      return (graph_->edges[uid_].first == e.graph_->edges[e.uid_].first
	      && graph_->edges[uid_].second == e.graph_->edges[e.uid_].second)
	|| (graph_->edges[uid_].second == e.graph_->edges[e.uid_].first
	    && graph_->edges[uid_].first == e.graph_->edges[e.uid_].second);
    }
     ...
     */
    bool operator<(const Edge& e) const {
      size_type sum_this = graph_->edges[uid_].first + graph_->edges[uid_].second;
      size_type sum_e = e.graph_->edges[e.uid_].first +  e.graph_->edges[e.uid_].second;
   ...
   private:
    ...
    Edge(const graph_type* graph, size_type i)
      : graph_(const_cast<graph_type *> (graph)), uid_(i) {
    }
  };
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (size_type i = 0; i < edges.size(); i++) {
      ...
      if ((cur.first == a.uid_ && cur.second == b.uid_) ||
	  (cur.second == b.uid_ && cur.first == a.uid_)) {
	return true;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    for (size_type i = 0; i < edges.size(); i++) {
      const auto &cur = edges[i];
      if ((cur.first == a.uid_ && cur.second == b.uid_) ||
	  (cur.second == a.uid_ && cur.first == b.uid_)) {
	return Edge(this, i);
      }
    }
