...
class Graph {
 ...
 public:
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // HW0: YOUR CODE HERE
    std::pair<size_type,size_type> edge1(a.idx, b.idx);
    std::pair<size_type,size_type> edge2(b.idx, a.idx);
    return (edge_hash_set.find(edge1) != edge_hash_set.end()) || 
           (edge_hash_set.find(edge2) != edge_hash_set.end()); 
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
    if(!has_edge(a,b)){
     std::pair<size_type,size_type> edge(a.idx, b.idx);
     edge_hash_set.insert(edge);
 ...
 private:
  ...
  //define a custom hash function so that std::pair<size_type, size_type>
  //can be used with std::unordered_set
  struct pair_hash {
   inline std::size_t operator()
   (const std::pair<size_type, size_type> &e) const {
    return e.first^e.second;
   }
  };
  
  std::unordered_set<std::pair<size_type, size_type>, pair_hash> edge_hash_set;
